...
PKGNAME = github.com/minio/minio
BASE_VERSION = 1.0.0
EXTRA_VERSION ?= $(shell git rev-parse  HEAD)
METADATA_VAR = minioVersion=$(BASE_VERSION)
...
docker: build checks
	@echo "Building minio docker image '$(TAG)'"
	@#docker build -t $(TAG) . -f Dockerfile.dev
	@docker build -t $(DOCKER_REPO)/minio:latest . -f Dockerfile
	docker push  $(DOCKER_REPO)/minio:latest
