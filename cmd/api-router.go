...
func registerAPIRouter(router *mux.Router) {
	...
	*/
	...
	for _, router := range routers {
		...
		// DeleteObjectACL todo : 不需要 delete objectacl
		//router.Methods(http.MethodDelete).Path("/{object:.+}").HandlerFunc(
		//	collectAPIStats("getobjectacl", maxClients(gz(httpTraceHdrs(api.DeleteObjectACLHandler))))).Queries("acl", "")
		// PutObjectACL
		router.Methods(http.MethodPut).Path("/{object:.+}").HandlerFunc(
		...
		router.Methods(http.MethodGet).Path("/{object:.+}").HandlerFunc(
			collectAPIStats("getDagTree", maxClients(gz(httpTraceAll(api.GetDagTreeHandler))))).Queries("dag", "")
		...
		//
		router.Methods(http.MethodPost).Path("/{object:.+}").HandlerFunc(
			collectAPIStats("postobject", maxClients(gz(httpTraceHdrs(api.PostObjectHandler)))))
		...
		// PutBucketACL
		router.Methods(http.MethodPut).HandlerFunc(
			collectAPIStats("putbucketacl", maxClients(gz(httpTraceAll(api.PutBucketACLHandler))))).Queries("acl", "{acl:.*}")
		// DeleteBucketACL
		router.Methods(http.MethodDelete).HandlerFunc(
			collectAPIStats("deletebucketacl", maxClients(gz(httpTraceAll(api.DeleteBucketACLHandler))))).Queries("acl", "")
		...
		// PutBucketLoggingHandler
		router.Methods(http.MethodPut).HandlerFunc(
			collectAPIStats("putbucketlogging", maxClients(gz(httpTraceAll(api.PutBucketLoggingHandler))))).Queries("logging", "")
		// DeleteBucketLoggingHandler
		router.Methods(http.MethodDelete).HandlerFunc(
			collectAPIStats("deletebucketlogging", maxClients(gz(httpTraceAll(api.DeleteBucketLoggingHandler))))).Queries("logging", "")
		...
		// GetBucketInfoDetail
		router.Methods(http.MethodGet).HandlerFunc(
			collectAPIStats("getbucketinfodetail", maxClients(gz(httpTraceAll(api.GetBucketInfoDetailHandler))))).Queries("bucketinfo", "")
		// PostPolicy
