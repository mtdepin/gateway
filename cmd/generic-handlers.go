...
func CORS(h http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// w.Header().Set("Access-Control-Allow-Origin", "https://panel.mty.wang:8000,https://panel.mty.wang,http://60.12.185.214")
		...
		head := ""
		for i := range commonS3Headers {
			head += "," + commonS3Headers[i]
		}
		w.Header().Set("Access-Control-Allow-Headers", head[1:])
		w.Header().Set("Access-Control-Allow-Methods", "POST, OPTIONS, GET, PUT, DELETE")

		if r.Method == "OPTIONS" {
			writeResponse(w, 206, nil, "")
			return
		}
		h.ServeHTTP(w, r)
	})
