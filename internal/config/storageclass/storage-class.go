...
type Config struct {
	...
	IA       StorageClass `json:"ia"`
	ARCHIVE  StorageClass `json:"archive"`
	CA       StorageClass `json:"ca"`
...
func Enabled(kvs config.KVS) bool {
	...
	iasc := kvs.Get(ClassIA)
	arsc := kvs.Get(ClassArchive)
	casc := kvs.Get(ClassCA)
	rrsc := kvs.Get(ClassRRS)
	return ssc != "" || iasc != "" || arsc != "" || casc != "" || rrsc != ""
}
