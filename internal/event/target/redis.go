...
// Save - saves the events to the store if questore is configured, which will be replayed when the redispool connection is active.
...
// send - sends an event to the redispool.
...
// Send - reads an event from store and sends it to redispool.
...
func NewRedisTarget(id string, args RedisArgs, doneCh <-chan struct{}, loggerOnce func(ctx context.Context, err error, id interface{}, errKind ...interface{}), test bool) (*RedisTarget, error) {
		...
		Dial: func() (redis.Conn, error) {
			...
			if args.Password != "" {
				if _, err = conn.Do("AUTH", args.Password); err != nil {
					cErr := conn.Close()
					targetID := event.TargetID{ID: id, Name: "redispool"}
	...
	target := &RedisTarget{
		id:         event.TargetID{ID: id, Name: "redispool"},
		args:       args,
		pool:       pool,
	...
	if args.QueueDir != "" {
		queueDir := filepath.Join(args.QueueDir, storePrefix+"-redispool-"+id)
