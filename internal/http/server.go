...
func (srv *Server) Start() (err error) {
	...
	wrappedHandler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if ok {
			ctx, span := trace.StartSpanWithRemoteParent(r.Context(), title, sc)
			span.AddAttributes(trace.StringAttribute("Host", r.Host))
			span.AddAttributes(trace.StringAttribute("RawQuery", r.URL.RawQuery))
			defer span.End()
			r = r.WithContext(ctx)
		} else {
			ctx, span := trace.StartSpan(r.Context(), title)
			span.AddAttributes(trace.StringAttribute("Host", r.Host))
			span.AddAttributes(trace.StringAttribute("RawQuery", r.URL.RawQuery))
			defer span.End()
			r = r.WithContext(ctx)
