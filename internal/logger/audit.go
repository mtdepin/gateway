...
func AuditLog(ctx context.Context, w http.ResponseWriter, r *http.Request, filterKeys ...string) {
	// Fast exit if there is not audit target configured
	if len(AuditTargets) == 0 {
		return
	}

	//reqClaims := cmd.GlobalIAMSys.GetAuthClaimsFromCtx(ctx)
	var reqClaims map[string]interface{} = nil
	var entry audit.Entry
