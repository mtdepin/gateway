...
import (
	...
	logConfig "github.com/minio/minio/maitian/config"
...
func LookupConfig(scfg config.Config) (Config, error) {
	...
	// 新增配置文件日志审计条目
	if logConfig.GetString("elasticsearch.enable") != "" {
		if logConfig.GetBool("elasticsearch.enable") {
			cfg.AuditWebhook["audit_webhook"] = http.Config{
				Enabled:  true,
				Endpoint: logConfig.GetString("elasticsearch.endpoint"),
			}
		}
	}
